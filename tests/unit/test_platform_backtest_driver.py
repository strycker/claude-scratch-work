...
def _fake_refit_l1(train_features: pd.DataFrame, cfg: dict, *, frozen_features: list[str] | None = None) -> pd.Series:
    ...
    Emits BOTH states (alternating) since plan 08-08: ``_fake_refit_l2``'s posterior
    names states 0 and 1, and a real nowcaster's ``classes_`` is always a subset of
    its window's labels. The old all-zero stand-in paired a state-1 posterior with a
    window that never saw state 1 — the combination ``regime_filter.likelihood_ratio``
    rejects as a wiring bug once the Bayes filter is on the default path.
    """
...
#: ``_fake_refit_l1`` / ``_varying_fake_refit_l2``, generated by running the driver.py
#: recovered from git (``git show 81bdc78:src/trading_crab_lib/platform/backtest/driver.py``,
#: the last commit to touch it before this plan) and stored as exact float hex.
_PRE_0808_CURVE_HEX = {
    "return": [
        "0x1.fcc07017cff38p-7", "0x1.e05a44359ff2cp-6", "-0x1.5893095f4e93cp-7", "-0x1.63814812c113ap-6",
        "-0x1.c7c856a333b85p-8", "0x1.b1a065666b757p-8", "0x1.0fa43f41d11c5p-9", "-0x1.db6f1e192b934p-7",
        "0x1.d1940e40d6abep-7", "-0x1.3d1c93b04c12dp-8", "-0x1.779739d22e8b9p-6", "-0x1.f370bc6bf62d0p-15",
    ],
    "turnover": [
        "0x1.0000000000000p+0", "0x1.43f1577944d68p-4", "0x1.f239701e17800p-10", "0x1.89aa98b099ca0p-4",
        "0x1.d077fb1a20c98p-4", "0x1.ae67ff22c00c0p-6", "0x1.d92f2921a5088p-3", "0x1.416cc8aa916d4p-4",
        "0x1.0ffe56623a16cp-4", "0x1.11a4e0a043c40p-5", "0x1.3697310f77170p-4", "0x1.12cafa00e5d3ap-2",
    ],
    "cost": [
        "0x1.0624dd2f1aa00p-10", "0x1.4bb7a5595d000p-14", "0x1.fe2e8749d2000p-20", "0x1.931d4862eb500p-14",
        "0x1.db9dad2917540p-14", "0x1.b8bc699c6a900p-16", "0x1.e48a678eff030p-13", "0x1.49239e60d6700p-14",
        "0x1.1685772d49d00p-14", "0x1.183625814b880p-15", "0x1.3e0b75d265700p-14", "0x1.19634dd3dd01ap-12",
    ],
    "active_regime": [
        "0x1.0000000000000p+0", "0x0.0p+0", "0x0.0p+0", "0x0.0p+0",
        "nan", "0x1.0000000000000p+0", "0x0.0p+0", "0x0.0p+0",
        "0x0.0p+0", "nan", "0x1.0000000000000p+0", "0x0.0p+0",
    ],
    "scale": [
...
def _pre_0808_curve() -> pd.DataFrame:
    idx = pd.date_range("2020-01-31", "2020-12-31", freq="ME", name="date")
    frame = pd.DataFrame(
        {c: [float.fromhex(v) if v != "nan" else float("nan") for v in vals] for c, vals in _PRE_0808_CURVE_HEX.items()},
        index=idx,
    )
...
class TestRegimeFilterInRunBacktest:
    def test_filter_off_reproduces_the_pre_change_curve_exactly(self, monkeypatch):
        """The escape hatch that keeps every pre-existing driver test meaningful."""
        curve, _ = _filter_run(monkeypatch, use_regime_filter=False)
        expected = _pre_0808_curve()
        expected.index = curve.index
        pd.testing.assert_frame_equal(curve, expected, check_exact=True)

    def test_filter_on_changes_the_curve_and_the_tilt_gets_the_belief(self, monkeypatch):
        seen = []
        real_tilt = driver.vol_targeted_tilt
        monkeypatch.setattr(driver, "vol_targeted_tilt", lambda p, *a, **k: seen.append(p) or real_tilt(p, *a, **k))
        curve, metrics = _filter_run(monkeypatch, use_regime_filter=True)
        expected = _pre_0808_curve()
        assert not np.array_equal(curve["return"].to_numpy(), expected["return"].to_numpy())
        assert len(seen) == len(metrics["belief"]) == len(metrics["proba"]) > 0
        n_differ = 0
        for probs, belief, raw, raw_classes in zip(seen, metrics["belief"], metrics["proba"], metrics["classes"]):
            np.testing.assert_array_equal(probs.to_numpy(), belief)
            raw_s = pd.Series(raw, index=raw_classes).reindex(probs.index, fill_value=0.0)
            n_differ += int(float(np.max(np.abs(raw_s.to_numpy() - belief))) > 1e-6)
        assert n_differ >= 1
    ...
    def test_the_tilt_off_ablation_is_never_filtered(self, monkeypatch):
        ...
        assert metrics["belief"] == [] and metrics["belief_classes"] == []
