"""
Phase D evaluation: impact of divergence features on regime classification.

Compares clustering quality and supervised model accuracy with and without
divergence features added to the feature pipeline.
...
Three evaluation axes:
  ...
  3. Transition detection — divergence feature behavior around known regime transitions

Usage:
    python scripts/evaluate_divergence.py
...
def _strip_divergence_from_config(cfg: dict) -> dict:
    """Return a config copy with divergence features removed from feature lists."""
    cfg2 = copy.deepcopy(cfg)
    div_prefixes = ("div_spy_", "div_cred_", "div_d1_")

    cfg2["features"]["initial_features"] = [
        f for f in cfg2["features"]["initial_features"]
        if not any(f.startswith(p) for p in div_prefixes)
    ]
    ...
    # Also remove sp500 raw level if it was only there for divergence
    # (keep it if it was already in the original baseline)
    # sp500 is a new addition for divergence support, remove it
    if "sp500" in cfg2["features"]["initial_features"]:
        cfg2["features"]["initial_features"].remove("sp500")
...
def _analyze_transitions(
    ...
    """Check divergence feature values around regime transition points."""
    feat = engineer_all(raw.copy(), cfg, causal=True)
    feat_numeric = feat.drop(columns=["market_code"], errors="ignore")
    ...
    div_cols = [c for c in feat_aligned.columns
                if any(c.startswith(p) for p in ("div_spy_", "div_cred_", "div_d1_"))]

    if not div_cols or len(transition_idx) == 0:
        return pd.DataFrame()

    # For each transition, collect divergence values at t-1, t, t+1
    rows = []
...
def main():
    ...
    cfg_no_div = _strip_divergence_from_config(cfg)

    print("=" * 72)
    print("PHASE D: Divergence Feature Impact Evaluation")
    print("=" * 72)
    ...
    feat_baseline = _build_features(raw, cfg_no_div)
    metrics_baseline = _cluster_metrics(feat_baseline, n_pca, k)

    print("Building features WITH divergence...")
    feat_divergence = _build_features(raw, cfg)
    metrics_divergence = _cluster_metrics(feat_divergence, n_pca, k)

    print(f"\n{'Metric':<25s} {'Baseline':>12s} {'+ Divergence':>12s} {'Delta':>10s}")
    print("-" * 60)
    ...
    sweep_divergence = _kmeans_sweep(feat_divergence, n_pca)

    print(f"\n{'k':>3s}  {'Sil (base)':>10s} {'Sil (+div)':>10s} {'Delta':>8s}  "
          f"{'CH (base)':>10s} {'CH (+div)':>10s} {'Delta':>8s}")
    print("-" * 70)
    for _, row_b in sweep_baseline.iterrows():
        k_val = int(row_b["k"])
        row_d = sweep_divergence[sweep_divergence["k"] == k_val].iloc[0]
    ...
    print("\nTraining RF WITHOUT divergence features...")
    sup_baseline = _supervised_cv(raw, cfg_no_div, labels)

    print("Training RF WITH divergence features...")
    sup_divergence = _supervised_cv(raw, cfg, labels)

    print(f"\n{'':>30s} {'Baseline':>12s} {'+ Divergence':>12s} {'Delta':>10s}")
    print("-" * 65)
    print(f"  {'Mean CV accuracy':<28s} {sup_baseline['mean_accuracy']:>12.4f} "
          f"{sup_divergence['mean_accuracy']:>12.4f} "
          f"{sup_divergence['mean_accuracy']-sup_baseline['mean_accuracy']:>+10.4f}")
    print(f"  {'Std CV accuracy':<28s} {sup_baseline['std_accuracy']:>12.4f} "
          f"{sup_divergence['std_accuracy']:>12.4f} "
          f"{sup_divergence['std_accuracy']-sup_baseline['std_accuracy']:>+10.4f}")
    print(f"  {'N features':<28s} {sup_baseline['n_features']:>12d} "
          f"{sup_divergence['n_features']:>12d}")
    print(f"  {'N samples':<28s} {sup_baseline['n_samples']:>12d} "
          f"{sup_divergence['n_samples']:>12d}")

    print("\nPer-fold accuracy:")
    for i, (b, d) in enumerate(zip(sup_baseline["per_fold"], sup_divergence["per_fold"])):
        print(f"  Fold {i+1}: baseline={b:.4f}  +div={d:.4f}  delta={d-b:+.4f}")

    # Feature importance comparison
    print("\nTop-15 features WITH divergence (RF importance):")
    for feat_name, imp in sup_divergence["top_features"].head(15).items():
        marker = " ** DIV **" if any(feat_name.startswith(p)
                                      for p in ("div_spy_", "div_cred_", "div_d1_")) else ""
        print(f"  {feat_name:<40s} {imp:.4f}{marker}")

    # ── 3. Transition detection ───────────────────────────────────────────
    print("\n" + "─" * 72)
    print("3. DIVERGENCE BEHAVIOR AROUND REGIME TRANSITIONS")
    print("─" * 72)

    trans_df = _analyze_transitions(raw, cfg, labels)
    if trans_df.empty:
        print("\n  No divergence features or transitions found in aligned data.")
    else:
        div_cols = [c for c in trans_df.columns
                    if any(c.startswith(p) for p in ("div_spy_", "div_cred_", "div_d1_"))]
        ...
        if trig_cols:
            ...
            for c in trig_cols:
                ...
                print(f"  {short:>12s}", end="")
            print()
            print("  " + "-" * (8 + 14 * len(trig_cols)))
            for offset in [-2, -1, 0, 1]:
                subset = trans_df[trans_df["offset_q"] == offset]
                print(f"  {offset:>+8d}", end="")
                for c in trig_cols:
                    rate = (subset[c].abs() > 0).mean()
    ...
    sil_delta = metrics_divergence["silhouette"] - metrics_baseline["silhouette"]
    acc_delta = sup_divergence["mean_accuracy"] - sup_baseline["mean_accuracy"]
    ...
    if trans_df.empty or not z_cols:
        print("  Transition detection: insufficient data for analysis")
    else:
        # Compare z-score magnitude at t=0 (transition) vs t=-2 (baseline)
        t0 = trans_df[trans_df["offset_q"] == 0]
        t_2 = trans_df[trans_df["offset_q"] == -2]
        for c in z_cols:
            z_at_t0 = t0[c].abs().mean()
            z_at_t2 = t_2[c].abs().mean()
            short = c.replace("div_", "").replace("_z_4q", "")
            if z_at_t0 > z_at_t2 * 1.2:
                verdict = "SIGNAL — elevated at transitions"
            elif z_at_t0 < z_at_t2 * 0.8:
                verdict = "NO SIGNAL — lower at transitions"
            else:
                verdict = "INCONCLUSIVE — similar magnitude"
            print(f"  {short} z-score at transition: {z_at_t0:.3f} vs baseline: {z_at_t2:.3f} → {verdict}")
