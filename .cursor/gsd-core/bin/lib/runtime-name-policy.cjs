 ...
 * ids fall back to 'Cursor' (the always-safe default, fail-closed).
 *
 ...
 * 'Cursor'. Sibling to `getDirName`; pure (no I/O).
 */
function getRuntimeLabel(runtime) {
    if (!runtime)
        return 'Cursor';
    const label = RUNTIME_LABELS[runtime];
    return typeof label === 'string' && label.length > 0 ? label : 'Cursor';
}
/**
