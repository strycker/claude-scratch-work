 ...
 * Cursor harness would use to create a 'fresh' parallel worktree. When
 ...
 * (origin/HEAD) that the Cursor harness would use when creating a 'fresh'
 * parallel worktree.
 *
...
function evaluateWorktreeBaseDegrade(deps) {
    ...
    // Cursor's worktree.baseRef accepts only "fresh" (= origin/HEAD, the default) or "head".
    ...
    // semantics and must be evaluated against origin/HEAD. (Reference: Cursor worktrees docs, #683.)
