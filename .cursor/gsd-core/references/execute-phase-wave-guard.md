0.5. **Inter-wave worktree base re-check (wave N+1 guard — #1369):**
   ...
   After Wave N merges and tracking commits advance orchestrator HEAD, Cursor's
   ...
   **To avoid this degrade across all waves:** set `worktree.baseRef:"head"` in
   ...
   Cursor to fork from the live HEAD instead of `origin/HEAD`, so each wave's new
