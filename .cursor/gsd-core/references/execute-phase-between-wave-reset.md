...
7c. **Between-wave manifest reset and worktree base refresh (waves 2+ only — #1369):**
   ...
   **REQUIRED before each wave transition when `USE_WORKTREES != "false"` and `RUNTIME = "claude"`.**
   ...
   Cursor harness may have cached as the worktree fork base at session start. New worktrees
   ...
   # Cursor harness re-reads the live HEAD on the next Agent(isolation="worktree") call
