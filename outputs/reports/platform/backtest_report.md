...
- median detection lag (months): 164.0
- **ratio: 0.5914634146341463**
...
- strategy: terminal log wealth=4.0265, max drawdown=-21.24%
...
- terminal log wealth delta (strategy - ablation): +0.3793 (strategy=4.0265, ablation=3.6472)
- max drawdown delta (strategy - ablation): -1.44% (strategy=-21.24%, ablation=-19.81%)

## Smoothed-vs-Filtered Gap

- gap (smoothed hindsight performance - real-time filtered performance): -0.4696 — the measured hindsight content of the strategy (§5.4). The smoothed reference is ONE full-sample labeler fit; the filtered series is the walk-forward driver's actual per-step decisions — genuinely distinct series (Pitfall 1), never the same object reused.
...
| Strategy (regime tilt) | 4.0265 | -21.24% |
| No-Regime Ablation | 3.6472 | -19.81% |

- no-regime-ablation delta vs. strategy: +0.3793 terminal log wealth (-1.44% max drawdown) — does the regime layer pay rent?

## Strategy KPIs

- terminal log wealth: 4.0265
- max drawdown: -21.24% (33 months underwater)
- CVaR(5%): -0.0463
- turnover (mean monthly): 0.0734
