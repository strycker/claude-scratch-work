...
- **Honesty discipline**: the fence is on *fitting*, not on *looking*. **Model fitting, tuning, and hyperparameter selection use only ≤2020-12 data** — enforced physically by the dev checkpoint manager, which cannot read the holdout tree. **Monitoring, scoring, and validation deliberately use the full span**, including post-2020: the platform exists to score current data for real trades, and a human must be able to see when a previously predictive feature has stopped working (e.g. a seasonality effect decaying). Reaching post-2020 data requires the explicit `get_holdout_checkpoint_manager()` opt-in, never a silent fallback. **When a post-2020 observation changes a decision, that decision is recorded with its date** — this is the residual honesty guarantee: the influence stays traceable even though it is no longer firewalled. Every evaluated configuration goes in the trial registry.

  *Amended 2026-08-04 (Phase 6 discussion).* The prior wording said post-2021 performance was "firewalled from all selection decisions until design freeze." That was unworkable in practice — catching feature decay is itself a selection decision informed by recent data, and refusing to make it would let a dead feature stay weighted. The trade is accepted knowingly: some bias risk, in exchange for not being blind to regime change in the live era.
